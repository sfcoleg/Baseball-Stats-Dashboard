...
- **Depth chart (starting lineup diamond) is also NOT part of the daily ingest** — `db.load_depth_chart(team_id)` hits `/api/v1/teams/{id}/roster?rosterType=depthChart` live (6-hour TTL cache), returning the current starter at each defensive position plus the rotation's #1 starting pitcher. Team abbreviation -> MLB team id mapping lives in `teams._TEAM_IDS` / `teams.team_id_for_abbr()`. Rendered by `style.baseball_diamond()` (HTML+SVG field with photo/name cards) on the Team page — reflects the team's *current* roster, not the season selected on that page.
