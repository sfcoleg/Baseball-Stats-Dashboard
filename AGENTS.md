...
app/
  ...
  pages/
    ...
    4_Team.py       # Full batting/pitching/fielding roster for one team+season, plus a starting-
                     ...
                     # "View" selector also offers three stats-driven composite rosters in place of a
                     # real team — All MLB / All Rookie / All Month — see db.build_composite_team()
...
- **Composite teams (All MLB / All Rookie / All Month, also on the Team page)** are built by `db.build_composite_team(season, mtime, scope)` — best qualified player (min 50 PA / 20 IP) at each of the 7 non-battery positions, sourced from the `fielding` table's `Pos` column (Statcast primary position) joined against `batting`/`pitching` (or `recent_batting`/`recent_pitching` "month" rows for the month scope). Catcher has no equivalent stats-based position source (Statcast OAA excludes the battery), so `db.load_league_catchers()` derives it by pulling every team's catcher off their live depth chart instead. "Rookie" here means age <= `db._ROOKIE_MAX_AGE` (23) — a heuristic, since this app has no MLB service-time data for actual rookie-eligibility status.
