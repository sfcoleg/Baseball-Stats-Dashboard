    ...
    if not spray.empty:
        ...
        if view_mode == "3D Trajectory":
            ...
            for outcome, group in spray.groupby("outcome"):
                # Plain Python lists, not pandas Series/DataFrame — passing
                # the latter through st.plotly_chart here mis-serializes (a
                # bad typed-array encoding that silently produces garbage-
                # scale coordinates), while the field-outline traces above,
                # built from plain lists, render fine.
                fig.add_trace(go.Scatter(
