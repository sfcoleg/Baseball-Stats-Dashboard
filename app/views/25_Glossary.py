...
_entry("dWAR (pitchers)", "Our own pitcher WAR, an in-house metric, and the one this site grades pitchers on. bWAR grades on runs actually allowed — which answers \"what happened\", defense and sequencing luck included. dWAR answers \"how good is this pitcher\" by running the same WAR machinery on EXPECTED run prevention instead: 0.8 × xFIP + 0.2 × xERA. xFIP normalises home runs to a league HR/FB rate, so a pitcher isn't blamed for fly balls that happened to carry; xERA adds Statcast's contact-quality estimate. Those weights were fitted, not chosen — every blend was scored on how well it predicts the FOLLOWING season's ERA across 948 paired pitcher-seasons, and 0.8/0.2 won (r=+0.393, against FIP's +0.343 and ERA's +0.260). The rate is then regressed toward league average by innings, lightly: a 45-inning xFIP is a small sample. That regression earns its place by keeping small-sample flukes off the top of the leaderboard rather than by sharpening prediction (it added only +0.003), and heavier regression measurably hurt. Needs Statcast, so it starts around 2015 — before that, bWAR stands in.")
_entry("bWAR (pitchers)", "Baseball-Reference's WAR — their published figure, not our calculation. It grades on runs actually allowed, so it does include the effect of the defense behind the pitcher and the order his hits fell in. Shown alongside dWAR because the two disagreeing is informative: a big gap usually means unusual defensive support or unusual sequencing luck. It also covers the seasons dWAR can't, going back to 1901 against dWAR's ~2015 Statcast limit.")
