...
_entry("fWAR (pitchers)", "Wins Above Replacement built on FIP rather than on runs actually allowed — it grades a pitcher on strikeouts, walks and home runs, so he isn't credited or blamed for the defense behind him or for sequencing luck. This is FanGraphs' published formula run on our own data; FanGraphs' site itself blocks automated access, so two pieces of theirs are missing: park factors (ours run neutral) and their final league-adjustment constant. Expect it to track fWAR closely without matching to the decimal.")
