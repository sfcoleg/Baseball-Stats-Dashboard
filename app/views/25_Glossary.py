...
_entry("wRAA", "Weighted runs above average — how many runs a batter produced beyond what a league-average hitter would have in the same number of plate appearances: (wOBA − league wOBA) ÷ wOBA scale × PA. It is the counting counterpart to wRC+: wRC+ says how good the rate was, wRAA says what that rate was worth given how often he actually batted, so a great hitter in 200 plate appearances and a good one in 650 can be compared on total contribution. Zero is exactly league average and the league total sums to zero by construction. Computed from this season's own field with a wOBA scale of 1.20, the same approximation wRC+ uses here, rather than FanGraphs' published per-season constant.")
