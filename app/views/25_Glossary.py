...
_entry("xFIP", "Like FIP, but home runs are replaced with fly balls × the league-average HR/FB rate — normalizes away a pitcher's own HR-per-fly-ball luck, good or bad, that regular FIP still bakes in.")

_entry("dWAR (position players)", "A complete position-player WAR \u2014 hitting, baserunning, fielding and a positional adjustment, not a batting-only number. The counterpart to pitcher dWAR, built the same way \u2014 on EXPECTED performance rather than what happened to land safely. Batting runs come from xwOBA (which prices contact quality, so a hitter isn\'t rewarded for balls that found grass or punished for lasers hit at someone) rather than wOBA, then baserunning runs, fielding runs, a positional adjustment and replacement level are added and the total is divided by runs-per-win. Two honest limits: fielding and baserunning data cover about 40% of all batters (though 83-91% of regulars with 300+ PA), and a player without a fielding row is treated as league-average afield rather than dropped \u2014 so for part-time players this is closer to a bat-and-position number. Catchers are absent from the fielding data entirely and are valued through pitch framing instead, which is the biggest measurable part of catcher defence but misses blocking and throwing.")
_entry("dWAR (pitchers)", "Our own pitcher WAR, and an in-house metric. bWAR grades on runs actually allowed and fWAR on realized FIP \u2014 both answer \"what happened\". dWAR answers \"how good is this pitcher\" by running the same WAR machinery on EXPECTED run prevention instead: 0.8 \u00d7 xFIP + 0.2 \u00d7 xERA. Those weights were fitted, not chosen \u2014 every blend was scored on how well it predicts the FOLLOWING season\'s ERA across 948 paired pitcher-seasons, and 0.8/0.2 won (r=+0.393, against FIP\'s +0.343 and ERA\'s +0.260). So it out-predicts the input fWAR is built on. The rate is then regressed toward league average by innings, lightly: a 45-inning xFIP is a small sample. That regression earns its place by keeping small-sample flukes off the top of the leaderboard rather than by sharpening prediction (it added only +0.003), and heavier regression measurably hurt. Needs Statcast, so it starts around 2015.")
_entry("fWAR (pitchers)", "Wins Above Replacement built on FIP rather than on runs actually allowed — it grades a pitcher on strikeouts, walks and home runs, so he isn't credited or blamed for the defense behind him or for sequencing luck. This is FanGraphs' published formula run on our own data, including a park adjustment built from our own six-season game log (Coors plays ~30% run-inflated; T-Mobile and Petco the other way). FanGraphs' site blocks automated access, so their exact figure can't be fetched — and one piece of theirs is missing here, the final league-adjustment constant that ties league-wide WAR to a target split with position players. Expect it to track fWAR closely without matching to the decimal.")
_entry("bWAR (pitchers)", "Baseball-Reference's WAR, which grades on runs actually allowed — so it does include the effect of the defense behind the pitcher. Shown alongside fWAR because the two disagreeing is informative: a big gap usually means unusual defensive support or unusual sequencing luck.")
_entry("xERA", "Expected ERA — what ERA \"should\" be based on the quality of contact allowed, not the actual results.")
_entry("PROP+ (Pitch Raw Output Profile)", "Our own pitch-quality model, and an in-house metric — 100 is league average, 10 points is one standard deviation, higher is better. It grades a pitch purely on its physical shape: velocity, vertical and horizontal break, how that movement compares to league average for that pitch type, spin rate, and how much it separates from the pitcher's own fastball. What the pitch actually DID — whiffs, strikeouts, wOBA, hard-hit rate — is deliberately excluded from the inputs and used only as what the model predicts, so PROP+ describes the pitch rather than its results. That makes it far stickier than outcomes: PROP+ predicts a pitch's run value next season better than its own run value this season does. Graded from 2020 on, where Statcast spin rate is available.")
