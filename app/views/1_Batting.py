...
if not _pd.empty and "total_pitches" in _pd.columns:
    ...
    _pd_cols = {
        "mlbID": "mlbID", "Eye": "Eye Score", "Contact": "Contact Score",
...
with statcast_tab:
    _sc_extra = [c for c in ("Power Score", "sweet_spot_percent") if c in table_rows.columns]
    display = teams.add_team_abbr(table_rows)[
    ...
    st.dataframe(
        style.style_stats_table(
            display,
            higher_better=["Power Score", "Sweet-Spot%", "Avg EV", "Max EV", "Hard-Hit%", "Barrel%",
                           "xBA", "xSLG", "wOBA diff"],
            team_col="Tm",
            team_color_fn=teams.color_for_abbr,
            precision={
                "Power Score": "{:.0f}", "Sweet-Spot%": "{:.1f}",
...
with discipline_tab:
    _eye_cols = [c for c in ("Eye Score", "Contact Score", "Chase Take%", "Shadow-out Take%",
                             "Shadow-in Swing%", "2-Strike Contact%") if c in table_rows.columns]
    have_eye = bool(_eye_cols) and table_rows["Eye Score"].notna().any()
    ...
    st.dataframe(
        style.style_stats_table(
            ...
            precision={
                ...
                "Eye Score": "{:.0f}", "Contact Score": "{:.0f}", "Chase Take%": "{:.1f}",
