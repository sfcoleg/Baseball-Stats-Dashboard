...
def win_probability_chart(wp_df, away_abbr: str, home_abbr: str, away_color: str, home_color: str) -> "go.Figure":
    ...
    chart. Plays that actually changed the score get a small marker (see
    scoring_rows below) so the chart reads as "here's where the game
    turned" instead of just a line — everything else is exactly one at-bat
    of many, but a scoring play is why the line moved."""
    fig = go.Figure()
    fig.add_hline(
